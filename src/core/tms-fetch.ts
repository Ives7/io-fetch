...
type Methods = keyof Pick<TmsFetch, 'get' | 'post' | 'put' | 'patch' | 'delete' | 'head'>;

const methods: Array<Methods> = ['post', 'get', 'patch', 'put', 'delete', 'head'];

export type SimpleMethod = <T = any, R = TmsResponse<T>>(
  url: string,
  config?: RequestConfig
) => Promise<R>;

export interface TmsFetch {
  post: SimpleMethod;
  delete: SimpleMethod;
  put: SimpleMethod;
  patch: SimpleMethod;
...
export class TmsFetch {
  constructor(private readonly defaultConfig: RequestConfig = {}) {
    this.interceptors = {
      request: new InterceptorManager<RequestConfig>(),
      response: new InterceptorManager<TmsResponse<any>>()
    };
  }

  public interceptors: {
    response: InterceptorManager<TmsResponse<any>>;
    request: InterceptorManager<RequestConfig>;
  };

  public request<T = any, R = TmsResponse<T>>(config: RequestConfig = {}): Promise<R> {
...
methods.forEach(method => {
  TmsFetch.prototype[method] = function<T = any, R = TmsResponse<T>>(
    url: string,
    config: RequestConfig = {}
  ): Promise<R> {
